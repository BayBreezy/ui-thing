<template>
  ...
  >
    ...
    <div
      class="flex h-[350px] w-full shrink-0 items-end bg-[url('https://images.unsplash.com/photo-1602345397613-0934a8812d23?q=80&w=1968&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')] bg-cover bg-no-repeat p-2 md:w-[600px] lg:h-full lg:w-1/2 lg:p-8"
    >
      <Swiper
        :autoplay="{ delay: 4000 }"
      ...
      >
        <SwiperSlide v-for="n in 4" :key="n">
          ...
          >
            <p class="mb-5 text-xl font-semibold lg:mb-8 lg:text-3xl">
              “We've been using Untitled to kick start every new project and can't imagine working
              without it.”
            </p>
            <div class="flex items-center justify-between">
              <p class="text-2xl font-semibold lg:text-3xl">Jane Doe</p>
              <div>
                <template v-for="s in 5" :key="s">
                  <Icon name="material-symbols:kid-star" class="h-5 w-5 text-white" />
                </template>
              </div>
            </div>
            <div class="mt-2 lg:mt-3">
              <p class="text-lg">COO, Swizzer Ltd</p>
