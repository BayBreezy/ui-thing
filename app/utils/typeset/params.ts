...
import { TYPESET_FONTS } from "./fonts";

export const TYPESET_PARAMS_MESSAGE = "typeset-params";
export const TYPESET_COMMAND_MESSAGE = "typeset-command";
/**
 * Sent by the preview once its listener is live. The iframe's `load` event fires before Vue
 * hydrates it, so a params message posted on `load` alone can arrive before anything is listening
 * and be dropped — which leaves the first paint showing defaults. The preview asks instead of the
 * designer guessing.
 */
...
/**
 * History snapshots restore through here: values are validated against the param model, unknown
 * keys are dropped, and absent keys become null so defaults clear.
 */
...
export function serializeTypesetSearchParams(base: string, params: TypesetSearchParams) {
