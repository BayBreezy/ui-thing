...
/**
 * Sort registrations by the position of their element in the document, so that navigation always
 * follows the rendered order instead of the mount order.
 */
...
/**
 * Map every enabled choice of an item definition to a keyboard shortcut, so that shortcuts stay
 * stable regardless of how choices are rendered.
 */
