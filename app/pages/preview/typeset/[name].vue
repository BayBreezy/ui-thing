...
<script lang="ts" setup>
  import type { FixtureName } from "~/utils/typeset/fixtures";
  import { AVAILABLE_CONTENT_OPTIONS, FIXTURES } from "~/utils/typeset/fixtures";
  import { CHAT_QUESTION } from "~/utils/typeset/fixtures/chat";
  import { findTypesetFont } from "~/utils/typeset/fonts";
  import { isEditableTarget } from "~/utils/typeset/keyboard";
  import { TYPESET_COMMAND_MESSAGE, TYPESET_MEASURES } from "~/utils/typeset/params";
  ...
  definePageMeta({
    ...
    validate(route) {
      return AVAILABLE_CONTENT_OPTIONS.some((option) => option.value === route.params.name);
    },
  });
  ...
  const headingFont = computed(() =>
    params.heading === "inherit" ? bodyFont.value : findTypesetFont(params.heading)
  );
  ...
  function forwardCommand(command: string) {
    if (window.parent && window.parent !== window) {
      window.parent.postMessage({ type: TYPESET_COMMAND_MESSAGE, command }, window.location.origin);
    }
  }
...
<style>
  /* typeset.css itself is imported globally from app/assets/css/tailwind.css. */
  html {
