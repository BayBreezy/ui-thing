<template>
  <fieldset
    ...
    :inert="!active"
    tabindex="-1"
    :class="questionnaireItemStyles({ class: normalizeClass(props.class) || undefined })"
  >
...
<script lang="ts">
  ...
  import type { AnswerControlRegistration, QuestionnaireItemStatus } from "~/utils/questionnaire";
  import {
  ...
  export const questionnaireItemStyles = tv({
    base: "flex min-w-0 flex-col gap-4 border-0 p-0 outline-none",
  });
  ...
  export type QuestionnaireItemProps = {
     ...
     */
    multiple?: boolean;
    /** Required. Submitted under this name, and used to activate the item. */
    name: string;
    /**
...
<script setup lang="ts">
  ...
  const props = withDefaults(defineProps<QuestionnaireItemProps>(), {
    disabled: false,
    invalid: false,
    multiple: false,
    required: false,
  ...
  /**
   * Selecting a controlled choice clears the native checked state of the other choices in the
   * group, so every control re-syncs after an interaction the host may have rejected.
   */
