<template>
  <div
    class="bg-background ring-foreground/10 relative isolate flex size-full min-h-0 flex-1 overflow-hidden rounded-2xl ring-1"
  >
    <iframe
      ref="iframeRef"
...
<script lang="ts" setup>
  import type { TypesetCommand, TypesetSearchParams } from "~/utils/typeset/params";
  import {
  ...
  function buildPreviewUrl() {
    return serializeTypesetSearchParams(`/preview/typeset/${params.item.value}`, params.toValues());
  }
  ...
  function sendParams() {
    iframeRef.value?.contentWindow?.postMessage(
      { type: TYPESET_PARAMS_MESSAGE, data: mergedParams.value, theme: colorMode.value },
  ...
  // The preview is a separate document, so it never sees @nuxtjs/color-mode
  // changes made in this window — re-send whenever the resolved theme flips,
  // regardless of whether that came from a click or a keyboard shortcut.
  watch(
    () => colorMode.value,
    () => sendParams()
  );
