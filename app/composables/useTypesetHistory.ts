...
import { parseTypesetSnapshot, TYPESET_PARAM_KEYS } from "~/utils/typeset/params";

/**
 * Undo/redo for the designer.
 *
 * The browser's own history is not usable here: every picker interaction writes with `replace` (see
 * useTypesetSearchParams), so there is nothing to pop. This keeps its own stack of URL snapshots
 * instead.
 *
 * Pass `record: true` from the owning page — exactly one caller must set up the recording watcher
 * and the keyboard shortcuts. Every other caller gets the shared state and the actions.
 */
