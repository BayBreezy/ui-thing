...
/**
 * How long the pointer must settle before a preview applies. Every mousemove over a picker item
 * re-arms this trailing timer, so nothing applies while the cursor is in motion. Clears are never
 * debounced: reverting on leave/Escape must feel instant.
 */
 ...
 *
 * Kept out of the URL on purpose: the URL is what the undo stack snapshots, so committing hovers
 * would record a phantom entry for every hovered item. The preview iframe receives these through
 * the same postMessage channel as committed params and reverts when cleared.
 */
