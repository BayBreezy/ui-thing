 ...
 *
 * Backed by `useState` so every LockButton and the shuffle action share one store without leaking
 * between SSR requests (a module-level ref would). Stored as an array rather than a Set because
 * useState serialises into the Nuxt payload.
 */
