...
import type { TypesetParamKey, TypesetSearchParams } from "~/utils/typeset/params";
import { coerceTypesetValue, TYPESET_DEFAULTS, TYPESET_PARAM_KEYS } from "~/utils/typeset/params";
...
/**
 * URL-backed state for the typeset designer. One query key per param, so the URL stays shareable
 * and human-readable.
 *
 * Reads are validated against the param model, so a hand-edited URL falls back to the default
 * instead of rendering something broken. Writes drop values equal to the default, keeping the URL
 * free of noise.
 *
 * Always writes with `replace`: every picker interaction would otherwise push a browser history
 * entry, and the designer keeps its own undo stack (see useTypesetHistory).
 */
export function useTypesetSearchParams() {
  ...
  /**
   * Applies a partial update. A `null` value clears the param back to its default, which is how
   * history snapshots restore params that were absent from the URL.
   */
  function setParams(next: Partial<{ [K in TypesetParamKey]: TypesetSearchParams[K] | null }>) {
    for (const key of TYPESET_PARAM_KEYS) {
      if (!(key in next)) {
        continue;
      }
